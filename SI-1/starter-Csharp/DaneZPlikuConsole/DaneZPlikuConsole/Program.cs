...
namespace DaneZPlikuConsole
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            bool wpisuj = false;
            int dlugosc = wczytaneDane[0].Length;
            int[] ilosc= new int[] { 0, 0 };
            for(int i=0; i < wczytaneDane.GetLength(0); i++)
            {
                if (i == 0) { klasy.Add( wczytaneDane[i][dlugosc-1]); }
                else {
                    for (int j = 0; j < klasy.Count; j++)
                    {
                        if(wczytaneDane[i][dlugosc-1] == klasy[j])
                        {
                            wpisuj = false;
                            break;
                        }
                    ...
                    if (wpisuj == true)
                    {
                        klasy.Add( wczytaneDane[i][dlugosc-1]);
                    }
                }
            }
            ...
            Console.WriteLine("Ilosc obiektow w klasach:");
            for(int i = 0; i < klasy.Count; i++) { 
                for(int j =0; j < wczytaneDane.GetLength(0); j++ )
                {
                    if(klasy[i] == wczytaneDane[j][dlugosc - 1])
                    {
                        ilosc[i] += 1;
                    }   
                }
            }
            Console.WriteLine(klasy[0] + " : " + ilosc[0]);

            Console.WriteLine(klasy[1] + " : " + ilosc[1]);
